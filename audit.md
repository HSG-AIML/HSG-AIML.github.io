...
description="Robust adversarial journal entry sampling: (a) the combination sampling map in $Z$ of the local posting amount attribute in Data-A, (b) the corresponding robustness sampling map, (c) the obtained adversarial sampling region q<sub>s</sub>(z<sub>k=14</sub>) combining (a) and (b) with $d<sub>&phi;</sub>(z) ≥ 0.568, and, (d) generated adversarial journal entries X<sub>Adv</sub> when sampling along the posting amount trajectory resulting in generated entries that exhibit an increased posting amount." %}
